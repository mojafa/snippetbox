...
func (m *SnippetModel) Insert(title, content, expires string) (int, error) {
	...
	// Use the Exec() method on the embedded connection pool to execute // statement. The first parameter is the SQL statement, followed by // title, content and expiry values for the placeholder parameters. // method returns a sql.Result object, which contains some basic
...
func (m *SnippetModel) Get(id int) (*models.Snippet, error) {
	...
	if err != nil {
		...
		// sql.ErrNoRows error. We use the errors.Is() function check for that // error specifically, and return our own models.ErrNoRecord error // instead.
...
// returns multiple results like a SELECT * or (a number)
func (m *SnippetModel) Latest() ([]*models.Snippet, error) {
	...
	// Iterate through the rows of the result set. If iteration over all the rows completes then the // resultset automatically closes itself and frees-up the underlying
	// database connection.
	for rows.Next() {
